<template>
  <footer class="border-t border-slate-600 mt-[150px] flex justify-between py-5 px-5">
    <RouterLink to="/" class="flex gap-3 items-center">
      <img src="/img/Code/frontend.jpg" alt="" class="w-[45px] rounded-full" />
    </RouterLink>
    <div class="flex gap-5">
      <a
        href="https://hh.ru/applicant/resumes/view?resume=08775528ff0dd8bcdd0039ed1f455342777341"
        target="_blank"
        class=""
      >
        <img src="/hhLogo.png" alt="" class="w-[45px]" />
      </a>
      <a href="https://github.com/ramyus889" target="_blank" class="">
        <img src="/github.svg" alt="" class="w-[45px]" />
      </a>
    </div>
  </footer>
</template>
