export const itemsProgressVue = [
  {
    id: 1,
    name: 'Vue.js - Nuxt.js',
    value: 84,
    delay: 100,
    headerName: [
      {
        id: 1,
        name: 'Composition API',
        namejs: [
          { name: 'ref()', value: 75 },
          { name: 'reactive()', value: 35 },
          { name: 'watch()', value: 55 },
          { name: 'computed()', value: 40 },
          { name: 'onMounted()', value: 55 },
          { name: 'defineProps()', value: 56 },
          { name: 'defineEmits()', value: 32 },
          { name: 'provide()', value: 35 },
          { name: 'inject()', value: 35 },
          { name: 'props', value: 70 },
          { name: 'template', value: 78 }
        ]
      },
      {
        id: 2,
        name: 'Directives',
        namejs: [
          { name: 'v-model', value: 75 },
          { name: 'v-on', value: 73 },
          { name: 'v-bind', value: 64 },
          { name: 'v-show', value: 66 },
          { name: 'v-for', value: 80 },
          { name: 'v-if, v-else, v-else-if', value: 76 },
          { name: 'v-slot', value: 42 },
          { name: 'v-once', value: 21 },
          { name: 'v-muted', value: 12 },
          { name: 'v-text', value: 56 },
          { name: 'v-html', value: 75 },
          { name: 'v-pre', value: 43 },
          { name: 'v-memo', value: 31 },
          { name: 'v-cloak', value: 13 }
        ]
      }
    ]
  },
  {
    id: 2,
    name: 'JavaScript',
    value: 53,
    delay: 150,
    headerName: [
      {
        id: 1,
        name: 'Functions',
        namejs: [
          { name: 'map()', value: 72 },
          { name: 'function()', value: 80 },
          { name: 'Стрелочные функции', value: 80 },
          { name: 'filter()', value: 18 },
          { name: 'fetch()', value: 60 },
          { name: 'JSON', value: 67 },
          { name: 'if else', value: 76 },
          { name: 'switch', value: 12 },
          { name: 'for', value: 45 },
          { name: 'while', value: 32 },
          { name: 'Object', value: 75 },
          { name: 'Array', value: 43 },
          { name: 'Async/Await', value: 54 },
          { name: 'try/catch', value: 54 },
          { name: 'Взаимодействие: alert, prompt, confirm ', value: 44 }
        ]
      },
      {
        id: 2,
        name: 'Операторы/События',
        namejs: [
          { name: 'Переменные', value: 97 },
          { name: 'Условные операторы', value: 78 },
          { name: 'onClick', value: 48 },
          { name: 'onChange', value: 23 },
          { name: 'onInput', value: 16 }
        ]
      }
    ]
  },
  {
    id: 3,
    name: 'HTML',
    value: 87,
    delay: 200,
    headerName: [
      {
        id: 1,
        name: 'Tag - частота использования тегов',
        namejs: [
          { name: '<template></template>', value: 100 },
          { name: '<div></div>', value: 100 },
          { name: '<p></p>', value: 10 },
          { name: '<h1-h6></h1-h6>', value: 4 },
          { name: '<a></a>', value: 86 },
          { name: '<button></button>', value: 96 },
          { name: '<header></header>', value: 87 },
          { name: '<footer></footer>', value: 85 },
          { name: '<main></main>', value: 5 },
          { name: '<hr/>', value: 21 },
          { name: '<input/>', value: 76 },
          { name: '<iframe/>', value: 5 },
          { name: '<textarea></textarea>', value: 10 },
          { name: '<progress></progress>', value: 2 },
          { name: '<murquee></murquee>', value: 3 },
          { name: '<meta></meta>', value: 80 },
          { name: '<ul></ul>', value: 12 },
          { name: '<li></li>', value: 12 },
          { name: '<ol></ol>', value: 12 }
        ]
      }
    ]
  },
  {
    id: 4,
    name: 'CSS',
    value: 90,
    delay: 250
  },
  {
    id: 5,
    name: 'PrimeVue',
    value: 77,
    delay: 100
  },
  {
    id: 6,
    name: 'Vue-Router',
    value: 56,
    delay: 150
  },
  {
    id: 7,
    name: 'Tailwind v3, v4',
    value: 95,
    delay: 200
  },
  {
    id: 8,
    name: 'Github',
    value: 86,
    delay: 250
  },
  {
    id: 9,
    name: 'npm',
    value: 64,
    delay: 300
  },
  {
    id: 10,
    name: 'Netlify',
    value: 70,
    delay: 350
  },
  {
    id: 11,
    name: 'Vuetify',
    value: 40,
    delay: 400
  },
  {
    id: 12,
    name: 'Vitest',
    value: 10,
    delay: 450
  }
];
